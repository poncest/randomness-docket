...
p <- ggplot() +
  ...
  annotate("text", x = 50, y = 37,
           label = "TYPICAL LONGEST DROUGHT",
           family = label_font, fontface = "bold",
           size = 4.4, color = blue, hjust = 0.5) +

  ## Percentile context labels -- lets the reader see 46/87 are the
  ## 10th/90th percentile, not the hard edges of "possible" (v2 review:
  ## the marker read as "barely inside" without this context)
  annotate("text", x = band_x0, y = 30,
           label = "10TH PERCENTILE",
           family = label_font, fontface = "plain",
           size = 2.6, color = muted, hjust = 0.5) +
  annotate("text", x = band_x1, y = 30,
           label = "90TH PERCENTILE",
           family = label_font, fontface = "plain",
           size = 2.6, color = muted, hjust = 0.5) +

  ## Endpoint numbers -- modest size now (v2 had these matching 48's
  ## size, which competed with the marker instead of framing it)
  annotate("text", x = band_x0, y = 24,
           label = q10,
           family = label_font, fontface = "bold",
           size = 5, color = ink, hjust = 0.5) +
  annotate("text", x = band_x1, y = 24,
           label = q90,
           family = label_font, fontface = "bold",
           size = 5, color = ink, hjust = 0.5) +
  ...
  ## completion bar, which misrepresents an uncertainty range)
  annotate("segment", x = band_x0, xend = band_x1, y = 17, yend = 17,
           color = blue, linewidth = 1.6, lineend = "round") +
  annotate("segment", x = band_x0, xend = band_x0, y = 15, yend = 19,
           color = blue, linewidth = 1.1) +
  annotate("segment", x = band_x1, xend = band_x1, y = 15, yend = 19,
           color = blue, linewidth = 1.1) +

  ## current-value marker: gold dot on the track, "Today: 48" below --
  ## distinct treatment from the endpoint numbers, reinforcing gold =
  ## observed / blue = expected the same way Panel 2's percentages do
  annotate("point", x = marker_x, y = 17, size = 4.2, color = gold) +
  annotate("text", x = marker_x, y = 10.5,
           label = "TODAY",
           family = label_font, fontface = "bold",
           size = 2.6, color = gold_text, hjust = 0.5) +
  annotate("text", x = marker_x, y = 4.5,
           label = current_val,
           family = display_font, fontface = "plain",
           size = 7, color = gold, hjust = 0.5) +

  ## -- Verdict (AFTER the chart, tightened to two crisp sentences) --
  annotate("text", x = 50, y = -6,
           label = "Forty-eight draws feels like a long time.",
           family = text_font, fontface = "plain",
           size = 4.4, color = ink, hjust = 0.5) +
  annotate("text", x = 50, y = -13,
           label = "In a fair lottery, it's well within the normal range for the longest current drought.",
           family = text_font, fontface = "plain",
           size = 3.6, color = body_txt, hjust = 0.5) +
...
ggsave(here("outputs/panel_03_droughts_v3_docket.png"), p,
       width = 8, height = 16.6, dpi = 300, bg = paper)

cat("Saved panel_03_droughts_v3_docket.png\n")
