...
p <- ggplot() +
  coord_cartesian(xlim = c(0, canvas_w), ylim = c(0, canvas_h), expand = FALSE, clip = "off") +
  ...
  ## Chart title -- precision fix: this measures the longest ACTIVE/
  ## CURRENT drought at a snapshot, not the longest drought ever
  ## observed historically (that's the separate longest_drought_headline
  ## stat, number 1's all-time 140). Different claim, wording now says so.
  annotate("text", x = 50, y = 41,
           label = "TYPICAL LONGEST CURRENT DROUGHT",
           family = label_font, fontface = "bold",
           size = 4.2, color = blue, hjust = 0.5) +
  
  ## Endpoint numbers -- no percentile labels (removed per review)
  annotate("text", x = band_x0, y = 34,
           label = q10,
           family = label_font, fontface = "bold",
           size = 5, color = ink, hjust = 0.5) +
  annotate("text", x = band_x1, y = 34,
           label = q90,
           family = label_font, fontface = "bold",
           size = 5, color = ink, hjust = 0.5) +
  
  ## Open interval track (NOT a filled rect -- reads as an uncertainty
  ## interval, not a completion bar)
  annotate("segment", x = band_x0, xend = band_x1, y = 27, yend = 27,
           color = blue, linewidth = 1.6, lineend = "round") +
  annotate("segment", x = band_x0, xend = band_x0, y = 25, yend = 29,
           color = blue, linewidth = 1.1) +
  annotate("segment", x = band_x1, xend = band_x1, y = 25, yend = 29,
           color = blue, linewidth = 1.1) +
  
  ## current-value marker: gold dot + tight Today/48 unit
  annotate("point", x = marker_x, y = 27, size = 4.2, color = gold) +
  annotate("text", x = marker_x, y = 21.5,
           label = "TODAY",
           family = label_font, fontface = "bold",
           size = 2.6, color = gold_text, hjust = 0.5) +
  annotate("text", x = marker_x, y = 17,
           label = current_val,
           family = display_font, fontface = "plain",
           size = 7, color = gold, hjust = 0.5) +
  
  ## -- Verdict: one punchy line -- moved well clear of y=0 (v4 bug:
  ## this sat exactly on the clip boundary and got cut in half).
  ## v5.2: shifted 9 -> 12 to clear the two-line footer below it (see
  ## next block) -- the two changed independently and needed re-checking
  ## against each other, not just against y=0 in isolation.
  annotate("text", x = 50, y = 12,
           label = "Long doesn't mean unusual.",
           family = text_font, fontface = "plain",
           size = 6, color = ink, hjust = 0.5) +
  
  ## -- Footer: methodology -- also moved to a safely positive y --
  ## v5.1 fix: single unbroken line overflowed the 8in canvas width and
  ## got clipped at both edges (same class of bug as the v4 y=0 clip,
  ## just horizontal) -- split into two lines.
  ## v5.2 fix: the two-line block is taller than the old one-liner and
  ## collided with the verdict text above it -- moved 6 -> 5 and paired
  ## with the verdict's 9 -> 12 move above, so the two text blocks
  ## (roughly y=9.8-14.2 and y=2.8-7.2) now clear each other with margin,
  ## and the footer's own bottom edge (~2.8) stays clear of the y=0
  ## boundary that caused the original v4 bug.
...
ggsave(here("outputs/panel_03_droughts_v5_docket.png"), p,
       width = 8, height = 14.6, dpi = 300, bg = paper)

cat("Saved panel_03_droughts_v5_docket.png\n")
