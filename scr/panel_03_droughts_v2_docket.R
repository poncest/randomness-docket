...
p <- ggplot() +
  ...
  ## Value labels -- 48 and 87 carry the visual weight (roughly Panel
  ## 2's stat-number scale), 46 stays small and off to the side
  annotate("text", x = marker_x, y = 26,
           label = current_val,
           family = display_font, fontface = "plain",
           size = 8.5, color = gold, hjust = 0.5) +
  annotate("text", x = band_x1, y = 26,
           label = q90,
           family = display_font, fontface = "plain",
           size = 8.5, color = ink, hjust = 0.5) +
  ...
  ## current-value marker: gold dot on the track
  annotate("point", x = marker_x, y = 17, size = 4.2, color = gold) +

  ## 46 -- kept for honesty, de-emphasized (small, quiet, off to the side)
  annotate("text", x = band_x0, y = 11,
           label = q10,
           family = label_font, fontface = "plain",
           size = 2.8, color = muted, hjust = 0.5) +

  ## -- Verdict (AFTER the chart, one sentence, feeling not claim) --
  annotate("text", x = 50, y = 2,
           label = paste0("Forty-eight draws feels like a long time \u2014 fair randomness\n",
                           "usually keeps a number waiting this long, or longer."),
           family = text_font, fontface = "plain",
           size = 4.0, color = ink, hjust = 0.5, lineheight = 1.1) +
...
ggsave(here("outputs/panel_03_droughts_v2_docket.png"), p,
       width = 8, height = 15.8, dpi = 300, bg = paper)

cat("Saved panel_03_droughts_v2_docket.png\n")
